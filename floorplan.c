...
/* Introduction Paragraph: 
			This program implements a slicing tree floorplan with modules at its leaves and cutlines at its internal nodes. The initial slicing tree is created by using the init_slicing_tree function. Other functions are called to determine whether each node is a leaf, an internal node, and in a subtree. Additional functions modify the tree for slicing (rotate,recut, swap_module, and swap_topology), encodes the floorplan into a 1D postfix expression (get_expression and postfix_traversal), and gets the total sum of resource of modules (get_total_resource). 
*/
...
int is_leaf_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
	if(ptr->left == NULL && ptr->right == NULL)		// check if both children are NULL to determine leave node
		return 1;
  return 0;
}
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
	if(ptr->left != NULL && ptr->right != NULL)		// check if both children are not NULL to determine internal node
		return 1; 
  return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
	if( b == a)										// check if b and a are equal 
		return 1;
	if (b == NULL)								// case to stop recursive call 
		return 0;	
	if (b->parent == a)						// check if b's parent is a
		return 1; 
	return is_in_subtree(a,b->parent);			// check recursively 
}
...
void rotate(node_t* ptr) {
  // TODO: 
	int temp = ptr->module->w; 				// create a temp ptr and set it to the module's width
	ptr->module->w = ptr->module->h; 	// set the module's width to its height
	ptr->module->h = temp;						// set the module's height to temp (its width)
}
...
void recut(node_t* ptr) {
  ...
  // TODO:
	if(ptr->cutline == V)				// check if the cutline is vertical
		ptr->cutline = H; 				// change to horizontal cutline
	else if(ptr->cutline == H)	// check if the cutline is horizontal
		ptr->cutline = V; 				// change to vertical cutline
  return;
}
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
	module_t *temp = a->module;				// create temp module
	a->module = b->module; 						// swap module a and b
	b->module = temp; 
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
	node_t *tempparentb = b->parent;	// make temporary pointer for b's parent
	if(a->parent->left == a){					// check if a is on its parent's left
			a->parent->left = b;					// swap the location of a with b
			b->parent = a->parent;} 			// reinitialize b's parent pointer
	else if(a->parent->right == a){		// same with a as a right child
			a->parent-> right = b; 
			b->parent = a->parent;}
	if(tempparentb->left == b){				// check if b is on its parents left and execute same steps
			tempparentb->left = a; 
			a->parent = tempparentb;}
	else if(tempparentb->right == b){	// same with b as a right child
			tempparentb->right = a;
			a->parent = tempparentb;
	}
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // TODO:
	...
	if(ptr->module != NULL){													// check if module pointer exists in expression unit 
		expression[*nth].module = ptr->module; 
		expression[*nth].cutline = UNDEFINED_CUTLINE;		// set cutline to UNDEFINED_CUTLINE
		*nth += 1;																			//iterate
	}
	else if(ptr->cutline != UNDEFINED_CUTLINE){				// check if the expression unit is a cutline type
		expression[*nth].module = NULL;									// set module to NULL
...
{
  // TODO:
	int total = 0; 	// initiailize total resource
	if(ptr == NULL)	// base case for recursion
		return 0;
	total = total + get_total_resource(ptr->left); 	// recursive through left side
	total = total + get_total_resource(ptr->right); // recurseive through right side
	if(ptr->left == NULL && ptr->right == NULL)			// check for leave node
		total = total + ptr->module->resource;				// add resource to total
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  // TODO:
	node_t* node = (node_t*)malloc(sizeof(node_t));	// allocate memory for a new node
	
	if(n == num_modules-1){								// base case for last module
		node->left = NULL; 
		node->right = NULL; 
...
// for internal nodes
		...
		node->cutline = V;
		node->parent = par;
// right child of internal nodes
		node->right = (node_t*)malloc(sizeof(node_t));		// allocate memory 
		node->right->cutline = UNDEFINED_CUTLINE; 
		node->right->module = &modules[n];
